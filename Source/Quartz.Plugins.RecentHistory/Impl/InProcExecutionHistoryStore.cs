...
{
    public string SchedulerName { get; set; } = "Untitled";
    ...
    private int _totalJobsExecuted, _totalJobsFailed;

    public Task<ExecutionHistoryEntry?> Get(string fireInstanceId)
    {
        lock (_data)
        {
            return _data.TryGetValue(fireInstanceId, out var entry)
                ? Task.FromResult<ExecutionHistoryEntry?>(entry)
                : Task.FromResult<ExecutionHistoryEntry?>(null);
        }
    }
